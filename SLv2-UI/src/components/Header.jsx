...
const Header = () => {
  ...
  return (
    <div className={`fixed top-0 left-0 w-full z-50 mb-200  border-b border-n-6 lg:bg-n-8/90 lg:backdrop-blur-sm ${
        openNavigation ? "bg-n-8" : "bg-n-8/90 backdrop-blur-sm"
        }`}>
      <div className="flex  items-center px-5 lg:px-6 xl:px-10 max-lg:py-4">
        <a className="flex w-[8rem] xl:mr-8" href="#sikiza">
        <img src={star} width={50} height={10} alt="sikiza" />
        <img src={logo} width={80} height={10} alt="sikiza" />

        </a>
        <nav className={` ${openNavigation ? 'flex' : 'hidden'} fixed top-[5rem] left-0 right-0 bottom-0 bg-n-8 lg:static lg:flex lg:mx-auto lg:bg-transparent`}>
                <div className="relative z-2 flex flex-col items-end justify-end m-auto  lg:flex-row ">
                    {navigation.map((item) => (
